...
abstract class View extends GameComponent {
  ...
  public update(
    gameTime: DOMHighResTimeStamp,
    coveredByOtherScreen?: boolean
  ): void {
  ...
  public getMenuEntryAt(position: Vector): number {
    ...
    for (let entry of this.menuEntries) {
      ...
      if (rect.contains(new Vector(position.x, position.y))) return index;
