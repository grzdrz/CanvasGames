...
class Player extends GameObject {
  ...
  public loadContent(viewManager: ViewManager): void {
    viewManager.onMouseDown.subscribe(this.handleMouseDown);
    viewManager.onMouseMove.subscribe(this.handleMouseDown);
    viewManager.onMouseUp.subscribe(this.handleMouseUp);

    viewManager.onMouseDown.subscribe(this.handlerSetPosition);
    viewManager.onMouseMove.subscribe(this.handlerSetPosition);
    viewManager.onMouseUp.subscribe(this.handlerUnhand);

    viewManager.onKeyDown.subscribe(this.handlerKeyDown);
    viewManager.onKeyUp.subscribe(this.handlerKeyUp);

    viewManager.onKeyUp.subscribe(this.handleChangeAmmo);
  }

  public unloadContent(viewManager: ViewManager): void {
    viewManager.onMouseDown.unsubscribe(this.handleMouseDown);
    viewManager.onMouseMove.unsubscribe(this.handleMouseDown);
    viewManager.onMouseUp.unsubscribe(this.handleMouseUp);

    viewManager.onMouseDown.unsubscribe(this.handlerSetPosition);
    viewManager.onMouseMove.unsubscribe(this.handlerSetPosition);
    viewManager.onMouseUp.unsubscribe(this.handlerUnhand);

    viewManager.onKeyDown.unsubscribe(this.handlerKeyDown);
    viewManager.onKeyUp.unsubscribe(this.handlerKeyUp);

    viewManager.onKeyUp.unsubscribe(this.handleChangeAmmo);
  }
  ...
  public jumpTimeStamp = 0;
  public update(gameTime: DOMHighResTimeStamp) {
    super.update(gameTime);

    if (this.jumpTimeStamp > 0) {
      /* debugger; */
      this.jumpTimeStamp -= window.GLOBAL_TIME_STAMP;
    }
  ...
  public handlerKeyDown = (eventArgs: EventArgs<IKeyData>) => {
    ...
    if (
      ...
      this.jumpTimeStamp = 0.2; // задержки между прыжками
      velocity.y -= 10;
    }
  ...
  public handlerSetPosition = (eventArgs: EventArgs<IMouseData>) => {
    ...
    Body.setPosition(this.body, {
      x: eventArgs.data.mousePosition.x,
      y: eventArgs.data.mousePosition.y,
    });
  };
